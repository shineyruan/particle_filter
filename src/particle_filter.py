...
class ParticleFiler():
    ...
    def __init__(self):
        ...
        self.laser_sub = rospy.Subscriber(rospy.get_param("~scan_topic", "/scan"),
                                          LaserScan, self.lidarCB, queue_size=1)
        self.odom_sub = rospy.Subscriber(rospy.get_param("~odometry_topic", "/odom"),
                                         Odometry, self.odomCB, queue_size=1)
    ...
    def publish_tf(self, pose, stamp=None):
        ...
        map_laser_pos -= np.dot(tf.transformations.quaternion_matrix(
            tf.transformations.unit_vector(map_laser_rotation))[:3, :3], laser_base_link_offset).T
    ...
    def sensor_model(self, proposal_dist, obs, weights):
        ...
        if self.RANGELIB_VAR == VAR_RADIAL_CDDT_OPTIMIZATIONS:
            ...
            if self.SHOW_FINE_TIMING and self.iters % 10 == 0:
                print("sensor_model: init: ", np.round((t_init - t_start) / t_total, 2),
                      "range:", np.round((t_range - t_init) / t_total, 2),
                      "eval:", np.round((t_eval - t_range) / t_total, 2),
                      "squash:", np.round((t_squash - t_eval) / t_total, 2))
    ...
    def MCL(self, a, o):
        ...
        if self.SHOW_FINE_TIMING and self.iters % 10 == 0:
            print("MCL: propose: ", np.round((t_propose - t) / t_total, 2),
                  "motion:", np.round((t_motion - t_propose) / t_total, 2),
                  "sensor:", np.round((t_sensor - t_motion) / t_total, 2),
                  "norm:", np.round((t_norm - t_sensor) / t_total, 2))
...
parser.add_argument(
    '--config', help='Path to yaml file containing config parameters. Helpful for calling node directly with Python for profiling.')
